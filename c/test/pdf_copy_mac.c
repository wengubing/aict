...
/**
 * 该程序是一个适用于 macOS 的控制台应用程序。
 * 功能：通过 AppleScript 弹出图形界面选择目录，递归提取所有 PDF 文件到目标目录。
 */

// 函数声明
void select_directory_macos(const char* prompt, char* path);
...
int main() {
    ...
    select_directory_macos("请选择包含 PDF 的源目录", source_path);
    ...
    // 2. 选择目标目录
    printf("\n提示：请在弹出的窗口中选择【目标目录】...\n");
    select_directory_macos("请选择存放 PDF 的目标目录", dest_path);
...
/**
 * 调用 macOS 的 AppleScript 弹出文件夹选择对话框
 */
void select_directory_macos(const char* prompt, char* path) {
    char command[2048];
    // 构造 AppleScript 命令
    snprintf(command, sizeof(command), 
             "osascript -e 'POSIX path of (choose folder with prompt \"%s\")' 2>/dev/null", prompt);
